#!/usr/bin/env python3
"""
Kaze (Wind) Fast Port Scanner - Creates Nmap Entries
====================================================

High-speed port scanner - Kage's parallel scanning partner.
Specializes in maximum throughput and high-volume scanning.

Kaze specializes in:
- Ultra-fast socket-based port scanning  
- High-volume parallel processing
- Service detection and version extraction
- Creating Nmap model entries for Bugsy to process
- Parallel threading (32 workers) for maximum speed
...
Author: EGO Revolution - Kaze (Wind Scout)
Version: 1.0.0 - High-Speed Parallel Threading
"""
...
from artificial_intelligence.personalities.reconnaissance.kage.nmap_xml_parser import NmapXMLParser

# Import nmap3 for simplified Nmap execution
try:
    import nmap3
    NMAP3_AVAILABLE = True
except ImportError:
    NMAP3_AVAILABLE = False
...
try:
    from ai_system.error_reporting_to_l import KageErrorReporter as KazeErrorReporter
except ImportError:
    # Optional error reporting - create dummy class if not available
    class KazeErrorReporter:
        @staticmethod
...
# Log nmap3 availability after logger is defined
if not NMAP3_AVAILABLE:
    logger.warning("nmap3 module not available - falling back to subprocess for Nmap execution")


class KazeNmapScanner:
    """
    Kaze's ultra-fast port scanning service.
    Creates real Nmap entries in the database for Bugsy to fingerprint.
    Specializes in high-speed, high-volume scanning.
    """
    
    def __init__(self, parallel_enabled: bool = True):
        """
        Initialize Kaze's port scanner.
        
        Args:
            parallel_enabled: Use parallel threading (32 workers vs 20)
        """
        self.parallel_enabled = parallel_enabled
        
        # Kaze's high-speed scanning configuration (more ports, balanced workers)
        self.default_ports = [21, 22, 23, 25, 53, 80, 110, 143, 443, 445, 993, 995, 3306, 3389, 5432, 5900, 8080, 8443, 8888, 9000, 9200, 27017]
        self.socket_timeout = 2.0  # Faster timeout for speed
        self.max_workers = 32 if self.parallel_enabled else 20  # Balanced workers for throughput
        ...
        logger.info(f"💨 Kaze scanner initialized ({'Parallel' if self.parallel_enabled else 'Sequential'} mode, {self.max_workers} workers, Tor: {'enabled' if self.tor_enabled else 'disabled'}, LLM: {'enabled' if self.llm_enabled else 'disabled'})")
        if self.nmap_knowledge:
            logger.info(f"📚 Loaded Nmap knowledge base ({self.nmap_knowledge.get('total_pages', 0)} pages)")
    
    def scan_egg_record(self, egg_record_id: str, ports: List[int] = None, scan_type: str = 'kaze_port_scan', write_to_db: bool = True, egg_record_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Scan an EggRecord and optionally create Nmap entries.
        
        Args:
            egg_record_id: EggRecord UUID string to scan
            ports: List of ports to scan (defaults to extended ports)
            scan_type: Type of scan (default: 'kaze_port_scan')
        ...
        """
        try:
            return self._scan_egg_record_impl(egg_record_id, ports, scan_type, write_to_db, egg_record_data)
    ...
    def _scan_egg_record_impl(self, egg_record_id: str, ports: List[int] = None, scan_type: str = 'kaze_port_scan', write_to_db: bool = True, egg_record_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        """
        # Get full EggRecord data - use provided data or fetch from DB
        target = None
        if egg_record_data:
            # Use provided data (from REST API)
            ...
            # Fetch from database (only if write_to_db is True or data not provided)
            if not write_to_db:
                logger.error(f"❌ EggRecord data required when write_to_db=False")
                return {
                    'success': False,
                    'error': 'EggRecord data required for REST API mode',
        ...
        logger.info(f"💨 Kaze scanning {target} ({len(ports_to_scan)} ports)")
        ...
        # Enhance open ports with Nmap version detection (if nmap is available)
        if open_ports:
            try:
                open_ports = self._enhance_with_nmap_version_detection(target, ip_address, open_ports)
            except Exception as e:
                logger.debug(f"⚠️  Nmap version detection failed (non-fatal): {e}")
                # Continue with original open_ports if enhancement fails
        ...
        # Create Nmap scan entries for Bugsy (only if write_to_db is True)
        ...
        elif open_ports and not write_to_db:
            logger.debug(f"  📤 Skipping database write (REST API mode) - returning {len(open_ports)} open ports")
        ...
        try:
            ...
            logger.info(f"💨 Kaze scan complete: {open_ports_count}/{ports_count} ports open in {duration:.2f}s")
    ...
    def batch_scan(self, egg_record_ids: List[str], ports: List[int] = None) -> List[Dict]:
        ...
        """
        logger.info(f"💨 Kaze batch scanning {len(egg_record_ids)} targets")
        ...
        logger.info(f"💨 Kaze batch complete: {successful}/{len(egg_record_ids)} successful, {total_ports} total open ports found")
    ...
    def _parallel_scan(self, ip: str, ports: List[int], target: str) -> List[Dict]:
        ...
        logger.info(f"💨 Kaze parallel scan: {len(ports)} ports with {self.max_workers} workers")
    ...
    def _sequential_scan(self, ip: str, ports: List[int], target: str) -> List[Dict]:
        ...
        if not isinstance(ports, list):
            ...
            ports = self.default_ports if hasattr(self, 'default_ports') else [80, 443]
        logger.info(f"💻 Kaze sequential scan: {len(ports)} ports")
    ...
    def _enhance_with_nmap_version_detection(self, target: str, ip_address: str, open_ports: List[Dict]) -> List[Dict]:
        """
        Enhance open ports with Nmap version detection using nmap3 module.
        Much simpler than subprocess approach.
        
        Args:
            target: Target hostname or IP
            ip_address: Resolved IP address
            open_ports: List of open port dictionaries from socket scanning
            
        Returns:
            Enhanced list of port dictionaries with Nmap data
        """
        if not open_ports:
            return open_ports
        
        # Use nmap3 if available, otherwise fall back to subprocess
        if not NMAP3_AVAILABLE:
            return self._enhance_with_nmap_version_detection_subprocess(target, ip_address, open_ports)
        
        # Extract port numbers
        port_numbers = [str(p.get('port', p) if isinstance(p, dict) else p) for p in open_ports]
        if not port_numbers:
            return open_ports
        ...
        try:
            ...
            # Use version detection with port specification
            logger.debug(f"🔍 Running nmap3 version detection on {target} ports: {port_str[:50]}...")
            
            result = nmap.nmap_version_detection(
                target,
                args=f'-p {port_str} -Pn --max-retries 6 --min-parallelism 7 --version-intensity 5'
            )
            
            # Extract port data from nmap3 result
            target_data = result.get(target, {})
            ports_data = target_data.get('ports', [])
            
            # Create mapping of port -> nmap data
            nmap_port_map = {}
            for port_info in ports_data:
                if port_info.get('state') == 'open':
                    port_num = int(port_info.get('portid', 0))
                    nmap_port_map[port_num] = port_info
            
            # Enhance original ports
            enhanced_ports = []
            for port_info in open_ports:
                port_num = port_info.get('port', port_info) if isinstance(port_info, dict) else port_info
                
                enhanced_port = port_info.copy() if isinstance(port_info, dict) else {'port': port_num}
                
                if port_num in nmap_port_map:
                    nmap_data = nmap_port_map[port_num]
                    service = nmap_data.get('service', {})
                    
                    # Add Nmap data
                    enhanced_port['product'] = service.get('product', '')
                    enhanced_port['version'] = service.get('version', '')
                    enhanced_port['service_version'] = service.get('version', '')
                    enhanced_port['extrainfo'] = service.get('extrainfo', '')
                    enhanced_port['cpe'] = service.get('cpe', [])
                    enhanced_port['scripts'] = nmap_data.get('scripts', [])
                    enhanced_port['reason'] = nmap_data.get('reason', '')
                    
                    # Update service info
                    if service.get('name'):
                        enhanced_port['service'] = service.get('name')
                        enhanced_port['service_name'] = service.get('name')
                    
                    # Update banner/extrainfo
                    if service.get('extrainfo'):
                        enhanced_port['banner'] = service.get('extrainfo')
                        enhanced_port['service_info'] = service.get('extrainfo')
                    
                    logger.debug(f"✅ Enhanced port {port_num} with nmap3 data: product={service.get('product', 'N/A')}")
                
                enhanced_ports.append(enhanced_port)
            
            if enhanced_ports:
                logger.info(f"🔍 Enhanced {len(enhanced_ports)} ports with nmap3 version detection")
            
            return enhanced_ports
            
        except Exception as e:
            logger.debug(f"nmap3 version detection error (non-fatal): {e}")
            # Fall back to subprocess if nmap3 fails
            return self._enhance_with_nmap_version_detection_subprocess(target, ip_address, open_ports)
    
    def _enhance_with_nmap_version_detection_subprocess(self, target: str, ip_address: str, open_ports: List[Dict]) -> List[Dict]:
        """
        ...
        try:
            ...
            if result.returncode != 0:
                logger.debug("Nmap not available, skipping version detection")
                return open_ports
        except Exception:
            logger.debug("Could not check for nmap, skipping version detection")
            return open_ports
        ...
        # Run Nmap -sV on discovered open ports
        try:
            nmap_cmd = [
                'nmap',
                '-sV',  # Version detection
                '--version-intensity', '5',  # Maximum intensity
                '-p', ','.join(port_numbers),  # Only scan discovered ports
                '--open',  # Only show open ports
                '-oX', '-',  # Output XML to stdout
                target
            ]
            
            logger.debug(f"🔍 Running Nmap version detection (subprocess): {' '.join(nmap_cmd[:5])}...")
            ...
            if result.returncode != 0:
                logger.debug(f"Nmap version detection failed: {result.stderr[:200]}")
                return open_ports
            
            # Parse XML output
            parser = NmapXMLParser()
            parsed_xml = parser.parse_nmap_xml(result.stdout)
            
            if 'error' in parsed_xml:
                logger.debug(f"Nmap XML parsing error: {parsed_xml['error']}")
                return open_ports
            
            # Convert to scan result format
            nmap_results = parser.convert_to_scan_result_format(parsed_xml)
            
            # Create a mapping of port -> nmap result
            nmap_port_map = {}
            for nmap_result in nmap_results:
                port_num = nmap_result.get('port')
                if port_num:
                    nmap_port_map[port_num] = nmap_result
            
            # Enhance original open_ports with Nmap data
            enhanced_ports = []
            for port_info in open_ports:
                port_num = port_info.get('port', port_info) if isinstance(port_info, dict) else port_info
                
                # Find matching Nmap result
                if port_num in nmap_port_map:
                    nmap_data = nmap_port_map[port_num]
                    # Merge Nmap data into port_info
                    enhanced_port = port_info.copy() if isinstance(port_info, dict) else {'port': port_num}
                    
                    # Add Nmap XML fields
                    enhanced_port['product'] = nmap_data.get('product', '')
                    enhanced_port['version'] = nmap_data.get('version', '')
                    enhanced_port['service_version'] = nmap_data.get('service_version', '')
                    enhanced_port['extrainfo'] = nmap_data.get('extrainfo', '')
                    enhanced_port['cpe'] = nmap_data.get('cpe', [])
                    enhanced_port['scripts'] = nmap_data.get('scripts', [])
                    enhanced_port['reason'] = nmap_data.get('reason', '')
                    
                    # Update service info if Nmap found more details
                    if nmap_data.get('service_name'):
                        enhanced_port['service'] = nmap_data.get('service_name')
                        enhanced_port['service_name'] = nmap_data.get('service_name')
                    
                    # Update banner/extrainfo
                    if nmap_data.get('extrainfo'):
                        enhanced_port['banner'] = nmap_data.get('extrainfo')
                        enhanced_port['service_info'] = nmap_data.get('extrainfo')
                    
                    enhanced_ports.append(enhanced_port)
                    logger.debug(f"✅ Enhanced port {port_num} with Nmap data: product={nmap_data.get('product', 'N/A')}")
                else:
                    # No Nmap data for this port, keep original
                    enhanced_ports.append(port_info)
            
            if enhanced_ports:
                logger.info(f"🔍 Enhanced {len(enhanced_ports)} ports with Nmap version detection")
            
            return enhanced_ports
            
        except subprocess.TimeoutExpired:
            logger.debug("Nmap version detection timed out")
            return open_ports
        except Exception as e:
            logger.debug(f"Nmap version detection error (non-fatal): {e}")
            return open_ports
    
    def _execute_nmap_with_techniques(
        self, 
        target: str, 
        ip_address: str, 
        ports_to_scan: List[int], 
        strategy: Dict[str, Any], 
        waf_detection: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """
        Execute Nmap scan using heuristics-based arguments from strategy.
        ...
        Args:
            ...
            waf_detection: WAF detection information
            
        Returns:
            Dict with 'success' (bool), 'scan_results' (list), or None if failed
        """
        if not strategy.get('nmap_arguments'):
            logger.debug("No Nmap arguments in strategy, cannot execute Nmap")
            return None
        ...
        try:
            result = subprocess.run(['which', 'nmap'], capture_output=True, text=True)
            if result.returncode != 0:
                logger.debug("Nmap not available, cannot execute with techniques")
                return None
        except Exception:
            logger.debug("Could not check for nmap")
            return None
        
        # Build Nmap command from strategy arguments
        nmap_cmd = ['nmap']
        
        # Add ports specification
        if ports_to_scan:
            port_str = ','.join(str(p) for p in ports_to_scan)
            nmap_cmd.extend(['-p', port_str])
        
        # Add arguments from strategy
        nmap_args = strategy.get('nmap_arguments', [])
        for arg in nmap_args:
            if isinstance(arg, dict):
                ...
                if flag:
                    ...
                    if value:
                        # Handle flags that need space vs equals
                        if flag in ['-D', '-S', '-e', '--source-port', '--data-string', '--scanflags']:
                            nmap_cmd.append(str(value))
                        elif '=' in flag or flag.startswith('--'):
                            # Some flags can use = format
                            if '=' not in flag:
                                nmap_cmd.append(str(value))
                            else:
                                # Already has = in flag
        ...
        # Always add XML output to stdout
        nmap_cmd.extend(['-oX', '-'])
        
        # Add target
        nmap_cmd.append(target)
        
        logger.info(f"🔍 Executing Nmap with heuristics: {' '.join(nmap_cmd[:10])}...")
        ...
        try:
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=300,  # 5 minute timeout for complex scans
                stderr=subprocess.PIPE
            )
            
            if result.returncode != 0:
                logger.warning(f"Nmap scan failed: {result.stderr[:200]}")
                return {
                    'success': False,
            ...
            if 'error' in parsed_xml:
                logger.warning(f"Nmap XML parsing error: {parsed_xml['error']}")
                return {
                    'success': False,
                    'error': parsed_xml['error'],
                    'scan_results': []
                }
            ...
            # Convert to expected format (matching socket scan results)
            scan_results = []
            for nmap_result in nmap_results:
                scan_result = {
                    'port': nmap_result.get('port', 0),
                    'status': nmap_result.get('state', 'open') if nmap_result.get('state') == 'open' else 'closed',
                    'protocol': nmap_result.get('protocol', 'tcp'),
                    'service_name': nmap_result.get('service_name', '') or nmap_result.get('service', ''),
                    'service_version': nmap_result.get('service_version', '') or nmap_result.get('version', ''),
                    'service_info': nmap_result.get('banner', '') or nmap_result.get('extrainfo', ''),
                    'product': nmap_result.get('product', ''),
                    'extrainfo': nmap_result.get('extrainfo', ''),
                    'cpe': nmap_result.get('cpe', []),
                    'scripts': nmap_result.get('scripts', []),
                    'reason': nmap_result.get('reason', ''),
                    'scan_time': 0,  # Nmap doesn't provide per-port timing
                    'timestamp': datetime.now().isoformat()
                }
                scan_results.append(scan_result)
            
            logger.info(f"✅ Nmap scan completed: {len([r for r in scan_results if r.get('status') == 'open'])} open ports found")
            ...
            logger.warning(f"Nmap execution error: {e}")
            import traceback
            logger.debug(f"Nmap error traceback: {traceback.format_exc()}")
            return {
                'success': False,
                'error': str(e),
                'scan_results': []
...
# Singleton instance
_kaze_scanner_instance = None

def get_kaze_scanner(parallel_enabled: bool = True):
    """Get Kaze scanner instance (singleton)."""
    global _kaze_scanner_instance
    
    if _kaze_scanner_instance is None:
        _kaze_scanner_instance = KazeNmapScanner(parallel_enabled=parallel_enabled)
    
    return _kaze_scanner_instance


def scan_egg_record(egg_record_id: str) -> Dict[str, Any]:
    """
    Scan an EggRecord using Kaze scanner.
