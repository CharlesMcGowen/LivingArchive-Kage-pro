...
class ScanLearningService:
    ...
    def _get_cache_key(self, key_type: str, *args) -> str:
        """Generate cache key."""
        return f"ash:learning:{key_type}:{':'.join(str(a) for a in args)}"
    ...
    def record_technique_result(self, target: str, waf_type: Optional[str],
        ...
        try:
            ...
            try:
                ...
                if 'ash_technique_effectiveness' not in inspector.get_table_names():
                    logger.warning("⚠️  Learning table ash_technique_effectiveness does not exist yet - run create_learning_tables.py")
                    # Try to create tables automatically
                    try:
                        from artificial_intelligence.personalities.reconnaissance.ash.scan_learning_models import Base
    ...
    def get_best_technique(self, target: str, waf_type: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                inspector = inspect(db.bind)
                if 'ash_technique_effectiveness' not in inspector.get_table_names():
                    logger.debug("Learning table ash_technique_effectiveness does not exist yet")
                    # Try to create tables automatically
                    try:
                        from artificial_intelligence.personalities.reconnaissance.ash.scan_learning_models import Base
                        Base.metadata.create_all(db.bind)
    ...
    def record_waf_detection(self, target: str, waf_info: Dict[str, Any],
        ...
        try:
            ...
            try:
                inspector = inspect(db.bind)
                if 'ash_waf_detections' not in inspector.get_table_names():
                    logger.warning("⚠️  Learning table ash_waf_detections does not exist yet - run create_learning_tables.py")
                    # Try to create tables automatically
                    try:
                        from artificial_intelligence.personalities.reconnaissance.ash.scan_learning_models import Base
                        Base.metadata.create_all(db.bind)
    ...
    def record_scan_result(self, target: str, technique_used: str,
        ...
        try:
            ...
            try:
                inspector = inspect(db.bind)
                if 'ash_scan_results' not in inspector.get_table_names():
                    logger.warning("⚠️  Learning table ash_scan_results does not exist yet - run create_learning_tables.py")
                    # Try to create tables automatically
                    try:
                        from artificial_intelligence.personalities.reconnaissance.ash.scan_learning_models import Base
                        Base.metadata.create_all(db.bind)
