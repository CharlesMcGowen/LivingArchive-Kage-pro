...
class KageNmapScanner:
    ...
    def scan_egg_record(self, egg_record_id: str, ports: List[int] = None, scan_type: str = 'kage_port_scan', write_to_db: bool = True, egg_record_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """
    ...
    def _scan_egg_record_impl(self, egg_record_id: str, ports: List[int] = None, scan_type: str = 'kage_port_scan', write_to_db: bool = True, egg_record_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        if open_ports and write_to_db:
            ...
            try:
                ...
                with db.cursor() as cursor:
                    ...
                    # Prepare open_ports JSONB data (include all Nmap XML fields)
                    ports_json = []
                    for result in open_ports:
                        port_data = {
                            'port': result.get('port', result) if isinstance(result, dict) else result,
                            'protocol': result.get('protocol', 'tcp') if isinstance(result, dict) else 'tcp',
                            'service': result.get('service_name') or result.get('service', '') if isinstance(result, dict) else '',
                            'version': result.get('service_version') or result.get('version', '') if isinstance(result, dict) else '',
                            'state': result.get('state', 'open') if isinstance(result, dict) else 'open',
                            'banner': result.get('service_info') or result.get('banner') or result.get('extrainfo', '') if isinstance(result, dict) else '',
                            # Nmap XML fields
                            'product': result.get('product', '') if isinstance(result, dict) else '',
                            'extrainfo': result.get('extrainfo', '') if isinstance(result, dict) else '',
                            'cpe': result.get('cpe', []) if isinstance(result, dict) else [],
                            'scripts': result.get('scripts', []) if isinstance(result, dict) else [],
                            'reason': result.get('reason', '') if isinstance(result, dict) else ''
                        }
                        # Add SSL certificate info for HTTPS ports
                        if isinstance(result, dict) and result.get('port') in [443, 8443] and ssl_analysis:
    ...
    def _execute_nmap_with_techniques(
        ...
        """
        Execute Nmap scan using nmap3 with heuristics-based arguments from strategy.
        Simplified version using nmap3 module - learning arguments are properly utilized.
        ...
        """
        nmap_arguments = strategy.get('nmap_arguments', [])
        ...
        # Use nmap3 if available, otherwise fall back to subprocess
        if not NMAP3_AVAILABLE:
            return self._execute_nmap_with_techniques_subprocess(target, ip_address, ports_to_scan, strategy, waf_detection)
        
        try:
            nmap = nmap3.NmapHostDiscovery()
            
            # Build ports string
            port_str = ','.join(str(p) for p in ports_to_scan) if ports_to_scan else None
            
            # Build args string from learning arguments - simplified approach
            args_parts = []
            
            # Add learning arguments from strategy
            for arg in nmap_arguments:
                if isinstance(arg, dict):
                    flag = arg.get('flag', '')
                    value = arg.get('value')
                    if flag:
                        args_parts.append(flag)
                        if value:
            ...
            if port_str:
                args_parts.extend(['-p', port_str])
            
            # Add common arguments for reliability
            args_parts.extend(['-Pn', '--max-retries', '6', '--min-parallelism', '7'])
            
            # Join arguments into string for nmap3
            args_str = ' '.join(args_parts)
            
            logger.info(f"🔍 Executing nmap3 with heuristics: {target} args: {args_str[:100]}...")
            
            # Execute version detection scan (includes port scanning)
            result = nmap.nmap_version_detection(target, args=args_str)
            
            # Extract results from nmap3 response
            target_data = result.get(target, {})
            ports_data = target_data.get('ports', [])
            ...
            logger.info(f"✅ nmap3 scan completed: {len(scan_results)} open ports found")
            
            return {
                'success': True,
                'scan_results': scan_results,
                'command': f'nmap3 version_detection {target} {args_str}',
                'raw_result': result
            }
            
        except Exception as e:
            logger.warning(f"nmap3 execution error: {e}")
            import traceback
            logger.debug(f"nmap3 error traceback: {traceback.format_exc()}")
            # Fall back to subprocess if nmap3 fails
            return self._execute_nmap_with_techniques_subprocess(target, ip_address, ports_to_scan, strategy, waf_detection)
    
    def _execute_nmap_with_techniques_subprocess(
        self, 
        target: str, 
        ip_address: str, 
        ports_to_scan: List[int], 
        ...
        logger.info(f"🔍 Executing Nmap with heuristics (subprocess): {' '.join(nmap_cmd[:10])}...")
