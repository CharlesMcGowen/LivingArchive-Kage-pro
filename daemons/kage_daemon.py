...
project_root = Path(__file__).parent.parent
logs_dir = project_root / 'logs' / 'kage'
logs_dir.mkdir(parents=True, exist_ok=True)
log_file = logs_dir / f'kage_daemon_{datetime.now().strftime("%Y%m%d")}.log'
...
file_formatter = logging.Formatter(
    '%(asctime)s [KAGE] %(levelname)s: %(message)s',
    datefmt='%Y-%m-%d %H:%M:%S'
)
...
class KageDaemon:
    ...
    def _get_eggrecords(self):
        ...
        for attempt in range(max_retries):
            try:
                url = f"{self.config.get_server_url()}/reconnaissance/api/daemon/kage/eggrecords/"
    ...
    def _submit_scan_result(self, eggrecord_id, target, scan_type, result):
        ...
        try:
            ...
            timeout_config = self.config.get_timeout_config()
            url = f"{self.config.get_server_url()}/reconnaissance/api/daemon/kage/scan/"
            data = {
